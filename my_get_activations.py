...
def gendescr_3inp(model, data, comstok, comlen, batchsize, config, strat, beamwidth, outfile, stopword,outdir):
    # right now, only greedy search is supported...
    fid = [*data.keys()]
    ...
    act1_softmax_val_path = outdir+'/viz/{}-{}-act1-stopword-{}.txt'.format(fid[0], outfile.split('.')[0], stopword)
    act2_softmax_val_path = outdir+'/viz/{}-{}-act2-stopword-{}.txt'.format(fid[0], outfile.split('.')[0], stopword)
    act1_prob_file = open(act1_softmax_val_path, 'w')
...
def gendescr_4inp(model, data, comstok, comlen, batchsize, config, strat, beamwidth, outfile, stopword,outdir):
    # right now, only greedy search is supported...
    fid = [*data.keys()]

    tdats, sdats, smls, coms = zip(*data.values())
    tdats = np.array(tdats)
    ...
    print(stopword)
    for i in range(1, stopword):
        results = model.predict([tdats, sdats, smls, coms], batch_size=batchsize)
        for c, s in enumerate(results):
            coms[c][i] = np.argmax(s)

    act1 = get_activations(model, [tdats, sdats, smls, coms], layer_name='activation_1')
    act2 = get_activations(model, [tdats, sdats, smls, coms], layer_name='activation_2')
    act3 = get_activations(model, [tdats, sdats, smls, coms], layer_name='activation_3')
    act1_softmax_val_path = outdir + '/viz/{}-{}-act1-stopword-{}.txt'.format(fid[0], outfile.split('.')[0], stopword)
    act2_softmax_val_path = outdir + '/viz/{}-{}-act2-stopword-{}.txt'.format(fid[0], outfile.split('.')[0], stopword)
    act3_softmax_val_path = outdir + '/viz/{}-{}-act3-stopword-{}.txt'.format(fid[0], outfile.split('.')[0], stopword)
    act1_prob_file = open(act1_softmax_val_path, 'w')
    act2_prob_file = open(act2_softmax_val_path, 'w')
    ...
    for j in act3:
        display_activations(j, 'pdats_activation', fid, act3_prob_file, outfile, stopword)
    act1_prob_file.close()
    act2_prob_file.close()
...
def display_activations(activation_maps, title, fid_list, act_prob_file, outfile, stopword):
    ...
    for i, activation_map in enumerate(activation_maps):
        fid = fid_list[i]
        img_path = '/nfs/projects/projcon/outdir/viz/{}-{}-{}-stopword-{}.pdf'.format(fid, outfile.split('.')[0], title, stopword)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data1', dest='dataprep', type=str, default='/nfs/projects/projcon')
    parser.add_argument('--data2', dest='dataprep2', type=str, default='/nfs/projects/projcon')
    ...
    seqdata = pickle.load(open('%s/dataset_random.pkl' % (dataprep), 'rb'))
    drop()
    ...
    prep('loading config... ')
    (modeltypewsdats, mwsdatsid,datrim, timestartwsdats) = modelfilewsdats.split('_')
    (timestartwsdats, extwsdats) = timestartwsdats.split('.')
    modeltypewsdats = modeltypewsdats.split('/')[-1]
    configwsdats = pickle.load(open(outdir+'/histories/'+modeltypewsdats+'_conf_'+datrim+'_'+timestartwsdats+'.pkl', 'rb'))
    num_inputswsdats = configwsdats['num_input']

    (modeltypewosdats, mwosdatsid,datrim2, timestartwosdats) = modelfilewosdats.split('_')
    (timestartwosdats, extwosdats) = timestartwosdats.split('.')
    modeltypewosdats = modeltypewosdats.split('/')[-1]
    configwosdats = pickle.load(open(outdir+'/histories/'+modeltypewosdats+'_conf_'+datrim2+'_'+timestartwosdats+'.pkl', 'rb'))
    ...
    for c, fid_set in enumerate(batch_sets):
        ...
        if configwsdats['batch_maker'] == 'datsonly':
            ...
            batch_resultswsdats = gendescr_3inp(modelwsdats, batch, comstok, comlen, batchsize, configwsdats, strat, beamwidth, outfilewsdats, stopword, outdir)
        elif configwsdats['batch_maker'] == 'ast_threed':
            batch_resultswsdats = gendescr_4inp(modelwsdats, batch, comstok, comlen, batchsize, configwsdats, strat, beamwidth, outfilewsdats, stopword, outdir)
        elif configwsdats['batch_maker'] == 'astprojcon':
            batch_resultswsdats = gendescr_4inp(modelwsdats, batch, comstok, comlen, batchsize, configwosdats, strat, beamwidth, outfilewosdats, stopword, outdir)
        ...
        elif configwsdats['batch_maker'] == 'pathast_threed':
            batch_resultswsdats = gendescr_pathast(modelwsdats, batch, comstok, comlen, batchsize, configwsdats, strat, beamwidth, outfilewsdats, stopword)
        else:
    ...
    #prep('loading sequences... ')
    #seqdata = pickle.load(open('%s/dataset_random.pkl' % (dataprep2), 'rb'))
    #drop()
    ...
    for c, fid_set in enumerate(batch_sets):
        ...
        if configwosdats['batch_maker'] == 'datsonly':
            ...
            batch_resultswosdats = gendescr_3inp(modelwosdats, batch, comstok, comlen, batchsize, configwosdats, strat, beamwidth, outfilewosdats, stopword,outdir)
        elif configwosdats['batch_maker'] == 'ast_threed':
            batch_resultswosdats = gendescr_4inp(modelwosdats, batch, comstok, comlen, batchsize, configwosdats, strat, beamwidth, outfilewosdats, stopword,outdir)
        elif configwosdats['batch_maker'] == 'astprojcon':
            batch_resultswosdats = gendescr_4inp(modelwosdats, batch, comstok, comlen, batchsize, configwosdats, strat, beamwidth, outfilewosdats, stopword,outdir)
        ...
        elif configwodats['batch_maker'] == 'pathast_threed':
            batch_resultswosdats = gendescr_pathast(modelwosdats, batch, comstok, comlen, batchsize, configwosdats, strat, beamwidth, outfilewosdats, stopword)
        else:
            print('error: invalid batch maker1')
            sys.exit()
